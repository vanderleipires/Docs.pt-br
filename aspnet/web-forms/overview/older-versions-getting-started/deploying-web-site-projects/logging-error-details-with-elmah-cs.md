...
> [!NOTE]
> A função de administrador e três usuários no sistema de Scott, Jisun e Alice - foram adicionados a [ *Configurando um site que usa serviços de aplicativos* tutorial](configuring-a-website-that-uses-application-services-cs.md). Os usuários Scott e Jisun são membros da função de administrador. Para obter mais informações sobre autenticação e autorização, consulte Meus [tutoriais de segurança de site](../../older-versions-security/introduction/security-basics-and-asp-net-support-cs.md).
