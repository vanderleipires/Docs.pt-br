...
O conteúdo e a estrutura do `Global.asax` arquivo criado pelo Visual Studio pode diferir um pouco dependendo se você estiver usando um projeto de aplicativo da Web (WAP) ou um projeto de Site da Web (WSP). Com um WAP, o `Global.asax` é implementada como dois arquivos separados - `Global.asax` e `Global.asax.vb`. O `Global.asax` arquivo não contém nada, mas uma `@Application` diretiva que faz referência a `.vb` arquivo; o evento de manipuladores de interesse são definidos no `Global.asax.vb` arquivo. Para WSPs, apenas um único arquivo é criado, `Global.asax`, e os manipuladores de eventos são definidos em um `<script runat="server">` bloco.
...
> [!NOTE]
> Ao implantar o aplicativo ASP.NET, você precisa copiar o `Global.asax` arquivo para o ambiente de produção. O `Global.asax.vb` arquivo, que é criado no WAP, não precisam ser copiados para a produção porque esse código é compilado no assembly do projeto.
