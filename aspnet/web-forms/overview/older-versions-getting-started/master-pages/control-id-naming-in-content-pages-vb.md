...
[![A página inclui três controles da Web: uma caixa de texto, botão e rótulo](control-id-naming-in-content-pages-vb/_static/image4.png)](control-id-naming-in-content-pages-vb/_static/image3.png)

**Figura 03**: A página inclui três controles da Web: uma caixa de texto, o botão e o rótulo ([clique para exibir a imagem em tamanho normal](control-id-naming-in-content-pages-vb/_static/image5.png))
...
Como o mestre de página em si serve como um contêiner de nomeação, os controles da Web definidos na página mestre também tem alterado renderizado `id` valores de atributo. Por exemplo, o `DisplayDate` rótulo são adicionados para a página mestra a [ *criar um Layout de todo o Site com páginas mestras* ](creating-a-site-wide-layout-using-master-pages-vb.md) tutorial tem o seguinte marcação processado:
...
[![É gerada uma NullReferenceException](control-id-naming-in-content-pages-vb/_static/image8.png)](control-id-naming-in-content-pages-vb/_static/image7.png)
...
Se você definir um ponto de interrupção no `SubmitButton_Click` manipulador de eventos, você verá que as chamadas para `FindControl` retornar `Nothing`. O `NullReferenceException` é gerado ao tentar acessar o `Age` da caixa de texto `Text` propriedade.

O problema é que `Control.FindControl` pesquisa somente *controle*do descendentes que estão no mesmo contêiner de nomeação. Como a página mestra constitui um novo contêiner de nomenclatura, uma chamada para `Page.FindControl("controlID")` nunca se interpõe entre o objeto de página mestra `ctl00`. (Consulte a Figura 4 para exibir a hierarquia de controle, que mostra o `Page` objeto como o pai do objeto da página mestra `ctl00`.) Portanto, o `Results` rótulo e `Age` caixa de texto não são encontrados e `ResultsLabel` e `AgeTextBox` são atribuídos valores de `Nothing`.
...
[![A duração do usuário é exibida no rótulo](control-id-naming-in-content-pages-vb/_static/image11.png)](control-id-naming-in-content-pages-vb/_static/image10.png)
...
Com esse código, retornar o `IDIssues.aspx` classe code-behind da página e comente atual `FindControl` chamadas de método. Substituí-las por chamadas para `Page.FindControlRecursive("controlID")`. O que é interessante sobre métodos de extensão é que eles aparecem diretamente dentro da IntelliSense nas listas suspensas. Como mostra a Figura 7, quando você digita `Page` e, em seguida, clique em período, o `FindControlRecursive` método foi incluído o IntelliSense suspensa junto com outros `Control` métodos de classe.


[![Métodos de extensão são incluídos nas IntelliSense listas suspensas](control-id-naming-in-content-pages-vb/_static/image14.png)](control-id-naming-in-content-pages-vb/_static/image13.png)

**Figura 07**: métodos de extensão são incluídos nas IntelliSense listas suspensas ([clique para exibir a imagem em tamanho normal](control-id-naming-in-content-pages-vb/_static/image15.png))
...
O código acima, o valor de injeta o `Age` da caixa de texto `ClientID` propriedade na chamada do JavaScript para `getElementById`. Se você visitar a página através de um navegador e exibir o código-fonte HTML, você encontrará o seguinte código JavaScript:
