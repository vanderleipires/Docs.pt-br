...
Em toda a série de tutoriais usamos DataSets tipados para implementar a camada de acesso a dados e objetos comerciais de nossa arquitetura em camadas. Conforme discutido na [primeiro tutorial](../introduction/creating-a-data-access-layer-cs.md), s o conjunto de dados tipado que DataTables funcionam como repositórios de dados, enquanto os TableAdapters atuar como wrappers para se comunicar com o banco de dados para recuperar e modificar os dados subjacentes. Os TableAdapters encapsular a complexidade envolvida ao trabalhar com o banco de dados e nos poupa de precisar escrever código para se conectar ao banco de dados, emitir um comando ou preencher os resultados em uma DataTable.

Há vezes, no entanto, quando precisamos burrow as profundezas do TableAdapter e escrever um código que funciona diretamente com os objetos do ADO.NET. No [de encapsulamento de modificações de banco de dados em uma transação](../working-with-batched-data/wrapping-database-modifications-within-a-transaction-cs.md) tutorial, por exemplo, adicionamos métodos ao TableAdapter para início, confirmação e reversão de transações do ADO.NET. Esses métodos usados interno, criado manualmente `SqlTransaction` objeto que foi atribuído ao TableAdapter s `SqlCommand` objetos.
