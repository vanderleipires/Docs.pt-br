...
O SomePage.aspx de URL no link foi automaticamente convertido em uma URL que incluído o tíquete de autenticação - não tivemos que gravar uma linha de código! O tíquete de autenticação de formulário será inserido automaticamente na URL para todos os hiperlinks que não começam com `http://` ou `/`. Não importa se o hiperlink é exibido em uma chamada para Response. Redirect, em um controle de hiperlink ou em um elemento âncora HTML (ou seja, `<a href="...">...</a>`). Como a URL não é algo como `http://www.someserver.com/SomePage.aspx` ou `/SomePage.aspx`, será inserido o tíquete de autenticação de formulários para nós.
