---
uid: web-forms/videos/migrating/intro-to-aspnet-for-coldfusion-developers-adding-aspnet-to-your-repertoire
title: "Introdução ao ASP.NET para desenvolvedores do ColdFusion: adicionando ASP.NET ao seu repertório | Microsoft Docs"
author: microsoft
description: "Este sessões mostra uma comparação de código do mundo real ainda simples entre ColdFusion e ASP.NET."
ms.author: aspnetcontent
manager: wpickett
ms.date: 07/25/2006
ms.topic: article
ms.assetid: 835a2484-e9e5-468e-9e43-5ae4e61531b6
ms.technology: dotnet-webforms
ms.prod: .net-framework
msc.legacyurl: /web-forms/videos/migrating/intro-to-aspnet-for-coldfusion-developers-adding-aspnet-to-your-repertoire
msc.type: video
ms.openlocfilehash: b24e042a28f51bcf3dbdcb049caa8d55abe5f080
ms.sourcegitcommit: 9a9483aceb34591c97451997036a9120c3fe2baf
ms.translationtype: MT
ms.contentlocale: pt-BR
ms.lasthandoff: 11/10/2017
---
<a name="intro-to-aspnet-for-coldfusion-developers-adding-aspnet-to-your-repertoire"></a>Introdução ao ASP.NET para desenvolvedores do ColdFusion: adicionando ASP.NET ao seu repertório
====================
por [Microsoft](https://github.com/microsoft)

Este sessões mostra uma comparação de código do mundo real ainda simples entre ColdFusion e ASP.NET.

[&#9654; Assista ao vídeo (minutos 49)](https://channel9.msdn.com/Blogs/ASP-NET-Site-Videos/intro-to-aspnet-for-coldfusion-developers-adding-aspnet-to-your-repertoire)

>[!div class="step-by-step"]
[Anterior](intro-to-aspnet-for-jsp-developers-building-applications.md)
[Próximo](introduction-to-aspnet-for-coldfusion-developers-building-an-aspnet-application.md)
