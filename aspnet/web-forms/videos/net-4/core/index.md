---
uid: web-forms/videos/net-4/core/index
title: Principal | Microsoft Docs
author: rick-anderson
description: Esta série de vídeos explora melhorias de núcleo no ASP.NET 4 como auto-start, arquivos de configuração de limpeza e a codificação HTML.
ms.author: aspnetcontent
manager: wpickett
ms.date: 11/14/2011
ms.topic: article
ms.assetid: 257686f7-7e10-4c4b-a2e9-299f93c0f917
ms.technology: dotnet-webforms
ms.prod: .net-framework
msc.legacyurl: /web-forms/videos/net-4/core
msc.type: chapter
ms.openlocfilehash: 052d0b2bf0b94f5eb8efc274e08297ba6b8c1104
ms.sourcegitcommit: 9a9483aceb34591c97451997036a9120c3fe2baf
ms.translationtype: HT
ms.contentlocale: pt-BR
ms.lasthandoff: 11/10/2017
ms.locfileid: "26526235"
---
<a name="core"></a>Núcleo
====================
> Esta série de vídeos explora melhorias de núcleo no ASP.NET 4 como auto-start, arquivos de configuração de limpeza e a codificação HTML.


- [Inicialização automática](aspnet-4-quick-hit-auto-start.md)
- [Limpar arquivos Web. config](aspnet-4-quick-hit-clean-webconfig-files.md)
- [IDs de cliente previsível](aspnet-4-quick-hit-predictable-client-ids.md)
- [Método utilitário HtmlEncoder](aspnet-4-quick-hit-the-htmlencoder-utility-method.md)
