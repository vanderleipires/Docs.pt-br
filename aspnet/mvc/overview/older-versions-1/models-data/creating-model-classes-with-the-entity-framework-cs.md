...
Lembre-se de salvar o modelo de dados de entidade depois de fazer uma modificação clicando no botão Salvar (o ícone de disquete). Nos bastidores, o Designer de entidade gera um conjunto de classes do c#. Você pode exibir essas classes abrindo o arquivo MoviesDBModel.Designer.cs da janela do Gerenciador de soluções.


Não modifique o código do arquivo desde que suas alterações serão substituídas na próxima vez que você usar o Designer de entidade. Se você deseja estender a funcionalidade das classes de entidade definidas no arquivo, você pode criar *classes parciais* arquivos separados em.
...
Para usar a classe theMoviesDBEntities no controlador Home, você deve importar o namespace MovieEntityApp.Models (*MVCProjectName*. Modelos).


O \_campo de banco de dados é usado na ação Index () para recuperar os registros da tabela de banco de dados de filmes. A expressão \_banco de dados. MovieSet representa todos os registros da tabela de banco de dados de filmes. O método ToList() é usado para converter o conjunto de filmes em uma coleção genérica de objetos de filme (lista&lt;filme&gt;).
...
O modo de exibição na lista 2 contém um **foreach** loop que itera por meio de cada registro de filme e exibe os valores das propriedades de título e diretor do registro do filme. Observe que um link de edição e exclusão é exibido ao lado de cada registro. Além disso, um link Adicionar filme aparece na parte inferior do modo de exibição (consulte a Figura 7).

**Figura 7 – o modo de exibição do índice**

![clip_image014](creating-model-classes-with-the-entity-framework-cs/_static/image7.jpg)

A exibição do índice é um *digitado exibição*. A exibição de índice inclui um &lt;% @ Page %&gt; diretiva com um *Inherits* atributo que usa a propriedade de modelo para uma coleção genérica de lista fortemente tipada de objetos de filme (lista&lt;filme).
...
**A listagem 3 – Controllers\HomeController.cs (Adicionar métodos)**
...
**A listagem 4 – Controllers\HomeController.cs (editar métodos)**
...
**Listando 6 – \Controllers\HomeController.cs (ação de exclusão)**
