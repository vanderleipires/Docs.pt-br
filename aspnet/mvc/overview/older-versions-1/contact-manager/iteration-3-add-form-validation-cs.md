...
Observe a chamada para o método auxiliar Html.ValidationSummary() que aparece logo acima do formulário HTML. Se há mensagens de erro de validação, esse método exibe mensagens de validação em uma lista com marcadores.

Observe que, além disso, as chamadas para Html.ValidationMessage() que aparecem ao lado de cada campo de formulário. O auxiliar ValidationMessage() exibe uma mensagem de erro de validação individuais. No caso da listagem 1, um asterisco é exibido quando há um erro de validação.

Por fim, o auxiliar Html.TextBox() automaticamente renderiza uma classe de folha de estilos em cascata quando há um erro de validação associado com a propriedade exibida pelo auxiliar. O auxiliar Html.TextBox() renderiza uma classe chamada **erro de validação de entrada**.

Quando você cria um novo aplicativo ASP.NET MVC, uma folha de estilos CSS de chamada é criada automaticamente na pasta de conteúdo. Esta folha de estilos contém as seguintes definições de classes CSS relacionadas à aparência das mensagens de erro de validação:
