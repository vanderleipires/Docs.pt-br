...
---
<a name="aspnet-signalr-hubs-api-guide---server-c"></a>Guia de API de Hubs do ASP.NET SignalR - servidor (c#)
====================
por [Patrick Fletcher](https://github.com/pfletcher), [Tom Dykstra](https://github.com/tdykstra)

> Este documento fornece uma introdução à programação do lado do servidor da API de Hubs de SignalR do ASP.NET para o SignalR versão 2, com exemplos de código demonstrando opções comuns.
> 
...
> 
> O SignalR também oferece uma API de nível inferior chamada conexões persistentes. Para obter uma introdução para o SignalR, Hubs e conexões persistentes, consulte [Introdução ao SignalR 2](../getting-started/introduction-to-signalr.md).
> 
...
- [Como registrar o SignalR middleware](#route)
...
- [Como criar e usar as classes de Hub](#hubclass)
    ...
    - [Hubs fortemente tipada](#stronglytypedhubs)
- [Como definir métodos na classe Hub que os clientes poderão chamar](#hubmethods)
    ...
    - [Relatar o andamento de invocações do método de hub](#progress)
...
Os componentes do servidor para o SignalR 2 somente estão disponíveis no .NET 4.5. Servidores que executam o .NET 4.0 devem usar o SignalR v1. x.

<a id="route"></a>

## <a name="how-to-register-signalr-middleware"></a>Como registrar o SignalR middleware

Para definir a rota que os clientes usarão para se conectar ao seu Hub, chame o `MapSignalR` método quando o aplicativo for iniciado. `MapSignalR`é um [método de extensão](https://msdn.microsoft.com/en-us/library/vstudio/bb383977.aspx) para o `OwinExtensions` classe. O exemplo a seguir mostra como definir a rota de Hubs de SignalR usando uma classe de inicialização OWIN.

[!code-csharp[Main](hubs-api-guide-server/samples/sample1.cs)]

Se você estiver adicionando funcionalidade SignalR para um aplicativo ASP.NET MVC, certifique-se de que a rota SignalR é adicionada antes de outras rotas. Para obter mais informações, consulte [Tutorial: Introdução ao SignalR 2 e 5 MVC](../getting-started/tutorial-getting-started-with-signalr-and-mvc.md).
...
- Habilite as chamadas entre domínios usando CORS ou JSONP clientes do navegador.
...
- O arquivo de proxies JavaScript gerado conterá proxies para todos os Hubs em um arquivo.
    ...
    Para obter informações sobre proxies de JavaScript, consulte [guia de API de Hubs de SignalR - cliente JavaScript - o proxy gerado e o que ele faz para você](hubs-api-guide-javascript-client.md#genproxy).
...
### <a name="strongly-typed-hubs"></a>Hubs fortemente tipada

Para definir uma interface para seus métodos de hub que seu cliente pode referência (e habilitar o Intellisense em seus métodos de hub), derivar seu hub de `Hub<T>` (introduzido no SignalR 2.1) em vez de `Hub`:
...
<a id="progress"></a>
### <a name="reporting-progress-from-hub-method-invocations"></a>Relatar o andamento de invocações do método de hub

SignalR 2.1 adiciona suporte para o [padrão de relatório de andamento](https://blogs.msdn.com/b/dotnet/archive/2012/06/06/async-in-4-5-enabling-progress-and-cancellation-in-async-apis.aspx) introduzido no .NET 4.5. Para implementar o relatório de andamento, definir um `IProgress<T>` parâmetro para o método de hub que seu cliente pode acessar:

[!code-csharp[Main](hubs-api-guide-server/samples/sample22.cs)]

Ao escrever um método de servidor de longa execução, é importante usar um padrão de programação assíncrono com Async / Await em vez de bloquear o thread de hub.
...
- Um usuário específico, identificado pela ID de usuário.
    ...
    Por padrão, isso é `IPrincipal.Identity.Name`, mas isso pode ser alterado por [registrar uma implementação de IUserIdProvider com o host global](mapping-users-to-connections.md#IUserIdProvider).
...
- Uma lista de nomes de usuário (introduzido no SignalR 2.1).
...
O `Groups.Add` e `Groups.Remove` métodos execute de forma assíncrona. Se você quiser adicionar um cliente a um grupo e enviar imediatamente uma mensagem para o cliente usando o grupo, você precisa garantir que o `Groups.Add` método terminar primeiro. O exemplo de código a seguir mostra como fazer isso.

**Adicionar um cliente a um grupo e, em seguida, esse cliente de mensagens**
...
No VB.NET ou em um hub fortemente tipados, o objeto de estado do chamador não pode ser acessado por meio de `Clients.Caller`; em vez disso, use `Clients.CallerState` (introduzido no SignalR 2.1):
...
Para manipular erros que ocorrem em seus métodos de classe de Hub, use um ou mais dos seguintes métodos:
...
- Criar um módulo de pipeline de Hubs que manipula o [OnIncomingError](https://msdn.microsoft.com/en-us/library/microsoft.aspnet.signalr.hubs.hubpipelinemodule.onincomingerror(v=vs.111).aspx) método. O exemplo a seguir mostra um módulo de pipeline que registra erros, seguidos do código em Startup.cs que insere o módulo no pipeline de Hubs.

    [!code-csharp[Main](hubs-api-guide-server/samples/sample61.cs)]

    [!code-csharp[Main](hubs-api-guide-server/samples/sample62.cs?highlight=4)]
- Use o `HubException` classe (introduzido no SignalR 2). Esse erro pode ser gerado de qualquer invocação de hub. O `HubError` construtor recebe uma mensagem de cadeia de caracteres e um objeto para armazenar dados de erro extra. SignalR irá serializar automaticamente a exceção e enviá-lo para o cliente, onde ele será usado para rejeitar ou não a invocação de método do hub.
    ...
    **Código de cliente JavaScript demonstrando a resposta para gerar um HubException em um hub**

    [!code-html[Main](hubs-api-guide-server/samples/sample64.html)]

    **Código de cliente .NET demonstrando a resposta para gerar um HubException em um hub**
...
O código a seguir no *Startup.cs* arquivo registra o módulo para executar o pipeline de Hub:
