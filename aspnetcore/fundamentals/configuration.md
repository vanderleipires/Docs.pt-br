...
É comum ter configurações diferentes para ambientes diferentes, por exemplo, desenvolvimento, teste e produção. O seguinte código adiciona dois provedores de configuração de três fontes:

1. Provedor JSON, lendo *appSettings. JSON*
2. Provedor JSON, lendo *appsettings.\< EnvironmentName >. JSON*
3. Provedor de variáveis de ambiente
...
Associar a classe personalizada em `ConfigureServices` no `Startup` classe:
