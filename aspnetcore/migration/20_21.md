...
* Substitua a referência de pacote para `Microsoft.AspNetCore.All` com uma referência de pacote para `Microsoft.AspNetCore.App`. Talvez você precise adicionar dependências que foram removidas da `Microsoft.AspNetCore.All`. Para obter mais informações, consulte <xref:fundamentals/metapackage#migrate> e <xref:fundamentals/metapackage-app>. Para aplicativos destinados ao .NET Framework:
  * Adicione referências de pacote individuais em vez de uma referência do metapacote.
  * Atualize cada referência de pacote a 2.1.
...
## <a name="rules-for-projects-targeting-the-shared-runtime"></a>Regras para projetos direcionados para o tempo de execução compartilhado

ASP.NET Core inclui os seguintes tempos de execução compartilhados:
...
* As vantagens anteriores provavelmente fazem isso **não** justificar a conversão para o a versão da RCL.
...
* Use o código de identidade da interface do usuário 2.0 sem alterações. Usando o código de identidade da interface do usuário 2.0 tem suporte total. Isso é uma boa abordagem quando foram feitas alterações significativo para o código de identidade gerado.
...
* [Criar o scaffolding de identidade](xref:security/authentication/scaffold-identity) em seu projeto:
  * Selecione os projetos saindo *layout. cshtml* arquivo.
...
* Excluir o scaffolder de identidade gerado `IdentityDbContext` derrived classe na *áreas/identidade/Data/* pasta.
* Excluir *Areas/Identity/IdentityHostingStartup.cs*
* Atualizar o *loginpartial* arquivo:
  ...
  * Atualização de `<form />` elemento `    <form asp-area="Identity" asp-page="/Account/Logout" asp-route-returnUrl="@Url.Page("/Index", new { area = "" })" method="post" id="logoutForm" class="navbar-right">`
  ...
  O código a seguir mostra o atualizada *loginpartial* arquivo:
...
* Mover *Pages* para *Pages/Shared/_Layout.cshtml*
...
### <a name="validationscriptspartialcshtml"></a>Validationscriptspartial

* *Validationscriptspartial* move para *Pages/Shared/_ValidationScriptsPartial.cshtml*
