# [Introdução](index.md)

# [Introdução](getting-started.md)
## [Criar um aplicativo Web](xref:mvc/razor-pages/index)
## [Criar uma API Web](xref:tutorials/first-web-api)

# [Tutoriais](tutorials/index.md)
# [Princípios básicos](fundamentals/index.md)
## [Inicialização de aplicativos](fundamentals/startup.md)
## [Middleware](fundamentals/middleware.md)
## [Trabalhando com arquivos estáticos](fundamentals/static-files.md)
## [Roteamento](fundamentals/routing.md)
## [Middleware de regravação de URL](fundamentals/url-rewriting.md)
## [Tratamento de erro](fundamentals/error-handling.md)
## [WebSockets](fundamentals/websockets.md)
## [Globalização e localização](fundamentals/localization.md)
## [Configuração](fundamentals/configuration.md)
## [Registro em log](fundamentals/logging.md)
## [Provedores de arquivo](fundamentals/file-providers.md)
## [Injeção de dependência](fundamentals/dependency-injection.md)
## [Trabalhando com vários ambientes](fundamentals/environments.md)
## [Hospedagem](fundamentals/hosting.md)
## [Estado de sessão e de aplicativo](fundamentals/app-state.md)
## [Servidores](fundamentals/servers/index.md)
### [Kestrel](fundamentals/servers/kestrel.md)
### [Módulo do ASP.NET Core](fundamentals/servers/aspnet-core-module.md)
### [WebListener](fundamentals/servers/weblistener.md)
### [HTTP.sys](fundamentals/servers/httpsys.md)
## [Recursos de solicitação](fundamentals/request-features.md)
## [OWIN (Open Web Interface para .NET)](fundamentals/owin.md)
## [Escolher entre o ASP.NET Core e o ASP.NET](choose-aspnet-framework.md)
## [Escolher entre o tempo de execução do .NET Core e o do .NET Framework](https://docs.microsoft.com/dotnet/articles/standard/choosing-core-framework-server)
## [Metapacote Microsoft.AspNetCore.All](xref:fundamentals/metapackage)

# [MVC](mvc/overview.md)
## [Páginas do Razor](xref:mvc/razor-pages/index)
### [Sintaxe Razor](mvc/views/razor.md)
## [Associação de modelos](mvc/models/model-binding.md)
### [Validação de modelo](mvc/models/validation.md)
## [Exibições](mvc/views/overview.md)
### [Sintaxe Razor](mvc/views/razor.md)
### [Exibir compilação](mvc/views/view-compilation.md)
### [Layout](mvc/views/layout.md)
### [Auxiliares de marcas](mvc/views/tag-helpers/intro.md)
#### [Criando auxiliares de marcas](mvc/views/tag-helpers/authoring.md)
#### [Usando os auxiliares de marcas em formulários](mvc/views/working-with-forms.md)
                            
#### [Auxiliares de marcas internos](mvc/views/tag-helpers/built-in/index.md)
##### [Auxiliar de marca de âncora](mvc/views/tag-helpers/built-in/AnchorTagHelper.md)
##### [Auxiliar de marca de cache](mvc/views/tag-helpers/built-in/CacheTagHelper.md)
##### [Auxiliar de marca de cache distribuído](mvc/views/tag-helpers/built-in/DistributedCacheTagHelper.md)
##### [Auxiliar de marca de ambiente](mvc/views/tag-helpers/built-in/EnvironmentTagHelper.md)
##### [Auxiliar de marca de imagem](mvc/views/tag-helpers/built-in/ImageTagHelper.md)

### [Exibições parciais](mvc/views/partial.md)
### [Injeção de dependência em exibições](mvc/views/dependency-injection.md)
### [Exibir componentes](mvc/views/view-components.md)
## [Controladores](mvc/controllers/actions.md)
### [Roteamento para ações do controlador](mvc/controllers/routing.md)
### [Uploads de arquivo](mvc/models/file-uploads.md)
### [Injeção de dependência em controladores](mvc/controllers/dependency-injection.md)
### [Testando controladores](mvc/controllers/testing.md)

## [Avançado](mvc/advanced/index.md)
### [Trabalhando com o modelo de aplicativo](mvc/controllers/application-model.md)
### [Filtros](mvc/controllers/filters.md)
### [Áreas](mvc/controllers/areas.md)
### [Partes do aplicativo](mvc/advanced/app-parts.md)
### [Associação de modelos personalizada](mvc/advanced/custom-model-binding.md)
### [Formatadores personalizados](mvc/advanced/custom-formatters.md)
### [Formatando dados de resposta](mvc/models/formatting.md)

## [Tutoriais](tutorials/index.md)

### [Criar um aplicativo Web de Páginas do Razor](xref:tutorials/razor-pages/index)
#### [Introdução a Páginas do Razor](xref:tutorials/razor-pages/razor-pages-start)
#### [Adicionando um modelo](xref:tutorials/razor-pages/modelz)
#### [Páginas do Razor geradas por scaffolding](xref:tutorials/razor-pages/page)
#### [LocalDB do SQL Server](xref:tutorials/razor-pages/sql)
#### [Atualizando as páginas](xref:tutorials/razor-pages/da1)
#### [Adicionando pesquisa](xref:tutorials/razor-pages/search)
#### [Adicionando um novo campo](xref:tutorials/razor-pages/new-field)
#### [Adicionando validação](xref:tutorials/razor-pages/validation)

### [Criar um aplicativo Web MVC no Windows](tutorials/first-mvc-app/index.md)
#### [Introdução](tutorials/first-mvc-app/start-mvc.md)
#### [Adicionando um controlador](tutorials/first-mvc-app/adding-controller.md)
#### [Adicionando uma exibição](tutorials/first-mvc-app/adding-view.md)
#### [Adicionando um modelo](tutorials/first-mvc-app/adding-model.md)
#### [Trabalhando com o LocalDB do SQL Server](tutorials/first-mvc-app/working-with-sql.md)
#### [Exibições e métodos do controlador](tutorials/first-mvc-app/controller-methods-views.md)
#### [Adicionando pesquisa](tutorials/first-mvc-app/search.md)
#### [Adicionando um Novo Campo](tutorials/first-mvc-app/new-field.md)
#### [Adicionando Validação](tutorials/first-mvc-app/validation.md)
#### [Examinando os métodos Details e Delete](tutorials/first-mvc-app/details.md)

### [Criar um aplicativo Web MVC em um Mac](tutorials/first-mvc-app-mac/index.md)
#### [Introdução](tutorials/first-mvc-app-mac/start-mvc.md)
#### [Adicionando um controlador](tutorials/first-mvc-app-mac/adding-controller.md)
#### [Adicionando uma exibição](tutorials/first-mvc-app-mac/adding-view.md)
#### [Adicionando um modelo](tutorials/first-mvc-app-mac/adding-model.md)
#### [Trabalhando com o SQLite](tutorials/first-mvc-app-mac/working-with-sql.md)
#### [Exibições e métodos do controlador](tutorials/first-mvc-app-mac/controller-methods-views.md)
#### [Adicionando pesquisa](tutorials/first-mvc-app-mac/search.md)
#### [Adicionando um Novo Campo](tutorials/first-mvc-app-mac/new-field.md)
#### [Adicionando Validação](tutorials/first-mvc-app-mac/validation.md)
#### [Examinando os métodos Details e Delete](tutorials/first-mvc-app/details.md)

### [Criar um aplicativo Web MVC com o VSCode](tutorials/first-mvc-app-xplat/index.md)
#### [Introdução](tutorials/first-mvc-app-xplat/start-mvc.md)
#### [Adicionando um controlador](tutorials/first-mvc-app-xplat/adding-controller.md)
#### [Adicionando uma exibição](tutorials/first-mvc-app-xplat/adding-view.md)
#### [Adicionando um modelo](tutorials/first-mvc-app-xplat/adding-model.md)
#### [Trabalhando com o SQLite](tutorials/first-mvc-app-xplat/working-with-sql.md)
#### [Exibições e métodos do controlador](tutorials/first-mvc-app-xplat/controller-methods-views.md)
#### [Adicionando pesquisa](tutorials/first-mvc-app-xplat/search.md)
#### [Adicionando um Novo Campo](tutorials/first-mvc-app-xplat/new-field.md)
#### [Adicionando Validação](tutorials/first-mvc-app-xplat/validation.md)
#### [Examinando os métodos Details e Delete](tutorials/first-mvc-app/details.md)

### [Criar um aplicativo Web de Páginas do Razor em um Mac](xref:tutorials/razor-pages-mac/index)
#### [Introdução a Páginas do Razor](xref:tutorials/razor-pages-mac/razor-pages-start)
#### [Adicionando um modelo](xref:tutorials/razor-pages-mac/model)


### [Criar um aplicativo Web de Páginas do Razor com o VSCode](xref:tutorials/razor-pages-vsc/index)
#### [Introdução a Páginas do Razor](xref:tutorials/razor-pages-vsc/razor-pages-start)
#### [Adicionando um modelo](xref:tutorials/razor-pages-vsc/model)


### [Acesso a dados – trabalhando com o EF Core](data/ef-mvc/index.md)
#### [Introdução](data/ef-mvc/intro.md)
#### [Operações de criação, leitura, atualização e exclusão](data/ef-mvc/crud.md)
#### [Classificação, filtragem, paginação e agrupamento](data/ef-mvc/sort-filter-page.md)
#### [Migrações](data/ef-mvc/migrations.md)
#### [Criando um modelo de dados complexo](data/ef-mvc/complex-data-model.md)
#### [Lendo dados relacionados](data/ef-mvc/read-related-data.md)
#### [Atualizando dados relacionados](data/ef-mvc/update-related-data.md)
#### [Tratando conflitos de simultaneidade](data/ef-mvc/concurrency.md)
#### [Herança](data/ef-mvc/inheritance.md)
#### [Tópicos avançados](data/ef-mvc/advanced.md)
### [Criando serviços de back-end para aplicativos móveis](mobile/native-mobile-backend.md)
### [Compilando APIs Web](mvc/web-api/index.md)
### [Criar uma API Web](xref:tutorials/first-web-api-mac)
#### [Páginas de ajuda da API Web ASP.NET usando o Swagger](tutorials/web-api-help-pages-using-swagger.md)
#### [Criando serviços de back-end para aplicativos móveis nativos](mobile/native-mobile-backend.md)

# [Teste e depuração](testing/index.md)
## [Testes de unidade](https://docs.microsoft.com/dotnet/articles/core/testing/unit-testing-with-dotnet-test)
## [Testes de integração](testing/integration-testing.md)
## [Testando controladores](mvc/controllers/testing.md)
## [Depuração remota](https://docs.microsoft.com/visualstudio/debugger/remote-debugging-azure)

# [Trabalhando com os Dados](data/index.md)
## [Introdução ao ASP.NET Core e Entity Framework Core usando o Visual Studio](data/ef-mvc/index.md)
## [ASP.NET Core com EF Core – novo banco de dados](https://docs.microsoft.com/ef/core/get-started/aspnetcore/new-db)
## [ASP.NET Core com EF Core – banco de dados existente](https://docs.microsoft.com/ef/core/get-started/aspnetcore/existing-db)
## [Introdução ao ASP.NET Core e Entity Framework Core 6](data/entity-framework-6.md)
## [Armazenamento do Azure](data/azure-storage/index.md)
### [Adicionando o Armazenamento do Azure usando o Visual Studio Connected Services](https://azure.microsoft.com/documentation/articles/vs-azure-tools-connected-services-storage)
### [Introdução ao Armazenamento de Blobs e ao Visual Studio Connected Services](https://azure.microsoft.com/documentation/articles/vs-storage-aspnet5-getting-started-blobs)
### [Introdução ao Armazenamento de Filas e ao Visual Studio Connected Services](https://azure.microsoft.com/documentation/articles/vs-storage-aspnet5-getting-started-queues)
### [Introdução ao Armazenamento de Tabelas e ao Visual Studio Connected Services](https://azure.microsoft.com/documentation/articles/vs-storage-aspnet5-getting-started-tables)

# [Desenvolvimento no Lado do Cliente](xref:client-side/index)
## [Usando o Gulp](xref:client-side/using-gulp)
## [Usando o Grunt](xref:client-side/using-grunt)
## [Gerenciar pacotes do lado do cliente com o Bower](xref:client-side/bower)
## [Criando sites bonitos e responsivos com o Bootstrap](xref:client-side/bootstrap)
## [Estrutura MVVM Knockout.js](xref:client-side/knockout)
## [Usando AngularJS para aplicativos de página única](xref:client-side/angular)
## [Usando JavaScriptServices para aplicativos de página única](xref:client-side/spa-services)
## [Criando estilo de aplicativos com Less, Sass e Font Awesome](xref:client-side/less-sass-fa)
## [Agrupamento e minificação](xref:client-side/bundling-and-minification)
## [Compilando Projetos com Yeoman](xref:client-side/yeoman)
## [Usando o link do navegador](xref:client-side/using-browserlink)

# [Dispositivo móvel](xref:mobile/index)
# [Criando Serviços de Back-end para Aplicativos Móveis Nativos](xref:mobile/native-mobile-backend)

# [Implantação e hospedagem](xref:publishing/index)
## [Hospedar no Windows com o IIS](xref:publishing/iis)
### [Referência de configuração do módulo do ASP.NET Core](xref:hosting/aspnet-core-module)
### [Módulos do IIS com o ASP.NET Core](xref:hosting/iis-modules)
## [Hospedar em um serviço Windows](xref:hosting/windows-service)
## [Hospedar em Linux com o Nginx](xref:publishing/linuxproduction)
## [Hospedar em Linux com o Apache](xref:publishing/apache-proxy)
## [Hospedar no Docker](xref:publishing/docker)
### [Compilando imagens do Docker](https://docs.microsoft.com/dotnet/articles/core/docker/building-net-docker-images)
### [Ferramentas do Visual Studio para Docker](https://docs.microsoft.com/dotnet/articles/core/docker/visual-studio-tools-for-docker)
### [Publicar para uma imagem do Docker](https://azure.microsoft.com/documentation/articles/vs-azure-tools-docker-hosting-web-apps-in-docker)
## [Publicar no Azure com o Visual Studio](xref:tutorials/publish-to-azure-webapp-using-vs)
## [Implantação contínua no Azure com o Visual Studio e o Git](xref:publishing/azure-continuous-deployment)
## [Implantação contínua no Azure com o VSTS](https://www.visualstudio.com/en-us/docs/build/aspnet/core/quick-to-azure)
## [Perfis de publicação no Visual Studio](xref:publishing/web-publishing-vs)
## [Estrutura de diretórios](xref:hosting/directory-structure)

# [Segurança](xref:security/index)
## [Autenticação](xref:security/authentication/index)
### [Opções de autenticação de OSS da comunidade](xref:security/authentication/community)
### [Introdução ao Identity](xref:security/authentication/identity)
### [Configurar o Identity](xref:security/authentication/identity-configuration)
### [Configurar a autenticação do Windows](xref:security/authentication/windowsauth)
### [Configurar o tipo de chave primária para o Identity](xref:security/authentication/identity-primary-key-configuration)
### [Provedores de armazenamento personalizado para o ASP.NET Core Identity](xref:security/authentication/identity-custom-storage-providers)
### [Habilitar a autenticação usando o Facebook, o Google e outros provedores externos](xref:security/authentication/social/index)
#### [Autenticação do Facebook](xref:security/authentication/facebook-logins)
#### [Autenticação do Twitter](xref:security/authentication/twitter-logins)
#### [Autenticação do Google](xref:security/authentication/google-logins)
#### [Autenticação da Microsoft](xref:security/authentication/microsoft-logins)
#### [Outros provedores de autenticação](xref:security/authentication/otherlogins)
### [Confirmação de conta e de recuperação de senha](xref:security/authentication/accconfirm)
### [Habilitar a geração de código QR no ASP.NET Identity](xref:security/authentication/identity-enable-qrcodes)
### [Autenticação de dois fatores com SMS](xref:security/authentication/2fa)
### [Usando a autenticação de cookie sem o ASP.NET Core Identity](xref:security/authentication/cookie)
### [Azure Active Directory](xref:security/authentication/azure-active-directory/index)
#### [Integrando o Azure AD em um aplicativo Web ASP.NET Core](https://azure.microsoft.com/documentation/samples/active-directory-dotnet-webapp-openidconnect-aspnetcore)
#### [Chamando uma API Web do ASP.NET Core de um aplicativo do WPF usando o Azure AD](https://azure.microsoft.com/documentation/samples/active-directory-dotnet-native-aspnetcore)
#### [Chamando uma API Web em um aplicativo Web ASP.NET Core usando o Azure AD](https://azure.microsoft.com/documentation/samples/active-directory-dotnet-webapp-webapi-openidconnect-aspnetcore)
### [Protegendo aplicativos ASP.NET Core com o IdentityServer4](https://identityserver4.readthedocs.io)
## [Autorização](xref:security/authorization/index)
### [Introdução](xref:security/authorization/introduction)
### [Criar um aplicativo com os dados de usuário protegidos por autorização](xref:security/authorization/secure-data)
### [Autorização simples](xref:security/authorization/simple)
### [Autorização baseada em função](xref:security/authorization/roles)
### [Autorização baseada em declarações](xref:security/authorization/claims)
### [Autorização baseada em política personalizada](xref:security/authorization/policies)
### [Injeção de dependência em manipuladores de requisitos](xref:security/authorization/dependencyinjection)
### [Autorização baseada em recurso](xref:security/authorization/resourcebased)
### [Autorização baseada em exibição](xref:security/authorization/views)
### [Limitação de identidade por esquema](xref:security/authorization/limitingidentitybyscheme)
## [Proteção de dados](xref:security/data-protection/index)
### [Introdução à proteção de dados](xref:security/data-protection/introduction)
### [Introdução às APIs de proteção de dados](xref:security/data-protection/using-data-protection)
### [APIs de consumidor](xref:security/data-protection/consumer-apis/index)
####[Visão geral de APIs do consumidor](xref:security/data-protection/consumer-apis/overview)
####[Cadeias de caracteres de finalidade](xref:security/data-protection/consumer-apis/purpose-strings)
####[Multilocação e hierarquia de finalidade](xref:security/data-protection/consumer-apis/purpose-strings-multitenancy)
####[Hash de senha](xref:security/data-protection/consumer-apis/password-hashing)
####[Limitando o tempo de vida de cargas protegidas](xref:security/data-protection/consumer-apis/limited-lifetime-payloads)
####[Desprotegendo cargas cujas chaves foram revogadas](xref:security/data-protection/consumer-apis/dangerous-unprotect)
###[Configuração](xref:security/data-protection/configuration/index)
####[Configurando a proteção de dados](xref:security/data-protection/configuration/overview)
####[Configurações padrão](xref:security/data-protection/configuration/default-settings)
####[Política para todo o computador](xref:security/data-protection/configuration/machine-wide-policy)
####[Cenários sem reconhecimento de DI](xref:security/data-protection/configuration/non-di-scenarios)
###[APIs de extensibilidade](xref:security/data-protection/extensibility/index)
####[Extensibilidade da criptografia de núcleo](xref:security/data-protection/extensibility/core-crypto)
####[Extensibilidade de gerenciamento de chaves](xref:security/data-protection/extensibility/key-management)
####[APIs diversas](xref:security/data-protection/extensibility/misc-apis)
###[Implementação](xref:security/data-protection/implementation/index)
####[Detalhes de criptografia autenticada.](xref:security/data-protection/implementation/authenticated-encryption-details)
####[Derivação de subchaves e criptografia autenticada](xref:security/data-protection/implementation/subkeyderivation)
####[Cabeçalhos de contexto](xref:security/data-protection/implementation/context-headers)
####[Gerenciamento de chaves](xref:security/data-protection/implementation/key-management)
####[Provedores de armazenamento de chaves](xref:security/data-protection/implementation/key-storage-providers)
####[Criptografia de chave em repouso](xref:security/data-protection/implementation/key-encryption-at-rest)
####[Imutabilidade de chave e alteração de configurações](xref:security/data-protection/implementation/key-immutability)
####[Formato de armazenamento de chaves](xref:security/data-protection/implementation/key-storage-format)
####[Provedores de proteção de dados efêmeros](xref:security/data-protection/implementation/key-storage-ephemeral)
###[Compatibilidade](xref:security/data-protection/compatibility/index)
####[Compartilhamento de cookies entre aplicativos](xref:security/data-protection/compatibility/cookie-sharing)
####[Substituindo <machineKey> no ASP.NET](xref:security/data-protection/compatibility/replacing-machinekey)
## [Impondo SSL](xref:security/enforcing-ssl)
## [Configurando HTTPS para desenvolvimento](xref:security/https)
## [Armazenamento seguro dos segredos do aplicativo durante o desenvolvimento](xref:security/app-secrets)
## [Provedor de configuração do Azure Key Vault](xref:security/key-vault-configuration)
## [Falsificação antissolicitação](xref:security/anti-request-forgery)
## [Evitando ataques de redirecionamento abertos](xref:security/preventing-open-redirects)
## [Evitando scripts entre sites](xref:security/cross-site-scripting)
## [Habilitando solicitações entre origens](xref:security/cors)

# [Desempenho](xref:performance/index)
## [Cache](xref:performance/caching/index)
### [Cache in-memory](xref:performance/caching/memory)
### [Trabalhando com um cache distribuído](xref:performance/caching/distributed)
### [Cache de resposta](xref:performance/caching/response)
### [Middleware de cache de resposta](xref:performance/caching/middleware)
## [Middleware de compactação de resposta](xref:performance/response-compression)

# [Migration](xref:migration/index) (Migração)
## [ASP.NET para ASP.NET Core 1.x](xref:migration/mvc)
### [Configuração](xref:migration/configuration)
### [Autenticação e identidade](xref:migration/identity)
### [API Web](xref:migration/webapi)
### [Módulos HTTP para Middleware](xref:migration/http-modules)
## [ASP.NET para ASP.NET Core 2.0](xref:migration/proper-to-2x/index)
## [ASP.NET Core 1.x para 2.0](xref:migration/1x-to-2x/index)
### [Autenticação e identidade](xref:migration/1x-to-2x/identity-2x)

# [Referência de API](/aspnet/core/api/)

# [Notas de versão 2.0](aspnetcore-2.0.md)
## [Notas de versão 1.1](aspnetcore-1.1.md)
## [Notas de versão anteriores](https://github.com/aspnet/home/releases)
## [Documentos do VS 2015/project.json](https://docs.microsoft.com/dotnet/articles/project-json)

# [Contribuir](https://github.com/aspnet/Docs/blob/master/CONTRIBUTING.md)
