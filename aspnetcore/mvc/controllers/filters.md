...
> [!IMPORTANT]
> Este tópico **não** se aplica a Páginas Razor. A versão prévia do ASP.NET Core 2.1 e as versões posteriores são compatíveis com [IPageFilter](/dotnet/api/microsoft.aspnetcore.mvc.filters.ipagefilter?view=aspnetcore-2.0) e [IAsyncPageFilter](/dotnet/api/microsoft.aspnetcore.mvc.filters.iasyncpagefilter?view=aspnetcore-2.0) para Páginas Razor. Para obter mais informações, confira [Métodos de filtro para Páginas Razor](xref:mvc/razor-pages/filter).
