...
---
# <a name="aspnet-core-with-iis-on-nano-server"></a>ASP.NET Core com IIS no Nano Server
...
3. Crie uma VM no Azure usando a imagem do Nano Server na Galeria do Azure. Se não tiver uma conta do Azure, você poderá obter uma versão de avaliação gratuita de 30 dias.

Neste tutorial estaremos usando a opção 2, o VHD do Nano Server pré-compilado do Windows Server 2016.

Antes de continuar com este tutorial, você precisará da [saída publicada](xref:hosting/directory-structure) de um aplicativo ASP.NET Core existente. Certifique-se de que seu aplicativo é criado para ser executado em um processo de **64 bits**.
...
## <a name="installing-the-aspnet-core-module-ancm"></a>Instalando o módulo ASP.NET Core (ANCM)

O módulo do ASP.NET Core é um módulo IIS 7.5+ responsável pelo gerenciamento de processos de ouvintes HTTP do ASP.NET Core e solicitações de proxy para processos que ele gerencia. No momento, o processo para instalar o módulo do ASP.NET Core para IIS é manual. Você precisará instalar o [pacote de hospedagem do Windows Server do .NET Core](https://download.microsoft.com/download/B/1/D/B1D7D5BF-3920-47AA-94BD-7A6E48822F18/DotNetCore.2.0.0-WindowsHosting.exe) em um computador normal (não Nano). Depois de instalar o pacote em um computador normal, você precisará copiar os arquivos a seguir para o compartilhamento de arquivos que criamos anteriormente.
...
Se você publicou um aplicativo dependente do Framework (portátil), o .NET Core deve ser instalado no computador de destino. Execute o script do PowerShell a seguir em uma sessão remota do PowerShell para instalar o .NET Framework em seu Nano Server.

> [!NOTE]
> Para entender as diferenças entre as FDDs (implantações dependentes do Framework) e SCDs (implantações autocontidas), consulte as [opções de implantação](https://docs.microsoft.com/dotnet/articles/core/deploying/).
...
O aplicativo Web publicado é acessível em um navegador em `http://192.168.1.10:8000`. Se você tiver configurado o log conforme descrito em [Criação e redirecionamento de Log](xref:hosting/aspnet-core-module#log-creation-and-redirection), você poderá exibir os logs em *C:\PublishedApps\AspNetCoreSampleForNano\logs*.
